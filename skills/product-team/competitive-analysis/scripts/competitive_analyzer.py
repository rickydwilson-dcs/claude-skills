...
def generate_report(results: dict, output_format: str) -> str:
    ...
    report.append("├─────────────────────────────────────┤")
    better_count = len(results.get('better', []))
    same_count = len(results.get('same', []))
    different_count = len(results.get('different', []))
    behind_count = len(results.get('behind', []))
    report.append(f"│  🟢 Better:    {better_count:>2} features          │")
    report.append(f"│  ✅ Same:      {same_count:>2} features          │")
    report.append(f"│  🟡 Different: {different_count:>2} features          │")
    report.append(f"│  ❌ Behind:    {behind_count:>2} features          │")
    report.append("│                                     │")
