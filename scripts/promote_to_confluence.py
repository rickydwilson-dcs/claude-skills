...
def promote_output(
    ...
    if not confluence_info:
        print(f"❌ Invalid Confluence URL format: {confluence_url}")
        print()
    ...
    print(f"   ✅ Found: {session_dir}")
    print()

    # Load session metadata
    metadata = load_session_metadata(session_dir)
    if not metadata:
        print("❌ Failed to load session metadata")
        return 1

    # Check if file exists in session
    source_file = session_dir / file_path
    if not source_file.exists():
        print(f"❌ File not found in session: {file_path}")
        print(f"   Looked in: {source_file}")
        return 1

    print(f"📄 Source file: {file_path}")
    print(f"🔗 Confluence: {space_key}/pages/{page_id}")
    print()
    ...
    try:
        ...
        print(f"❌ Failed to write promotion metadata: {e}")
        return 1
    ...
    if updated:
        if save_session_metadata(session_dir, metadata):
            ...
            print(f"⚠️  Failed to update session metadata")
    else:
        print(f"⚠️  Output not found in session metadata")
    ...
    print("=" * 70)
    print()
    print(f"✅ Output promoted successfully!")
    print()
    print(f"📋 Promoted file: {dest_file}")
    print(f"📝 Metadata: {promotion_metadata_path}")
    print(f"🔗 Confluence: {confluence_url}")
    print()

    if notify:
        print(f"📧 Notifications tracked for: {notify}")
