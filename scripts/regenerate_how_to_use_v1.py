...
def generate_example_invocations(skill_name: str, workflows: List[str], overview: str) -> List[str]:
    """Generate realistic example invocations"""
    skill_display = skill_name.replace('-', ' ').title()

    examples = []

    # Example 1: Basic usage from overview
    if overview:
        first_sentence = overview.split('.')[0].strip()
        action = first_sentence.lower()
        if 'provides' in action:
            action = action.split('provides')[-1].strip()
        elif 'helps' in action:
            action = action.split('helps')[-1].strip()
        examples.append(f'Hey Claude—I just added the "{skill_name}" skill. Can you help me {action}?')
    else:
        examples.append(f'Hey Claude—I just added the "{skill_name}" skill. Can you analyze my current codebase?')

    # Example 2: From first workflow
    if workflows:
        workflow = workflows[0].lower()
...
def generate_how_to_use(skill_path: Path) -> str:
    ...
    domain = metadata.get('metadata', {}).get('domain', skill_path.parent.name)
    skill_display = skill_name.replace('-', ' ').title()
